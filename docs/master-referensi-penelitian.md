...
**Referensi Window Dilemma:** Gower-Winter et al. (2026), *Advances in Intelligent Data Analysis XXIV* (IDA 2026), Springer LNCS vol. 16513, DOI: 10.1007/978-3-032-23833-7_27.  
 
...
Catatan rekonsiliasi: hitungan Wasserstein 273/312 merefleksikan seluruh jendela evaluasi detektor Fase 2. Hitungan retraining Fase 3--4 menggunakan 271/311 karena zona simulasi dimulai dari baris 241 setelah target-lag trimming dan pengecualian masa pemanasan.
...
* **Bukti Empiris yang Konsisten dengan Tesis The Window Dilemma:** Pelepasan alarm KS\_60 dan Wasserstein\_60 secara serempak di akhir September 2010 (tepat saat masa buffer baris integer 2W habis) konsisten dengan tesis Window Dilemma Gower-Winter et al. (2026). Dalam studi ini, ukuran jendela batch bertindak sebagai lensa buatan; penumpukan pergeseran tersembunyi dari bulan-bulan sebelumnya tampak muncul saat gerbang komputasi jendela pertama kali dibuka.
...
Eksperimen Fase 4 menunjukkan secara empiris bahwa ε-MAPE naif tidak aman untuk target finansial yang dekat nol. Pada hari bursa tanggal **30 November 2017**, kondisi pasar yang sangat stagnan membuat nilai aktual Log\_Return IHSG berada sangat dekat dengan angka nol mutlak. Dengan ε=1e-8 yang jauh lebih kecil daripada skala umum log-return harian 1e-2 hingga 1e-3, nilai eror ε-MAPE milik skenario pemicu kuartalan meledak hingga **190.081.112%**.  
**Keputusan Ilmiah Baku:** Metrik MAPE resmi **DIGUGURKAN** sebagai metrik akurasi akhir artikel ilmiah. Evaluasi dialihkan menggunakan metrik Mean Absolute Error (MAE) dan Root Mean Square Error (RMSE), sementara ledakan ε-MAPE dilaporkan sebagai kontribusi metodologis tentang bahaya MAPE pada target finansial dekat nol.

### **4.2. Matriks Metrik Akurasi & Varians Prediksi Akhir**

MAE saja tidak dapat membedakan prediktor yang benar-benar adaptif dari prediktor yang runtuh menjadi ramalan hampir konstan; karena itu standar deviasi prediksi (σ) diperlakukan sebagai diagnostik pendamping wajib dalam tabel utama.
